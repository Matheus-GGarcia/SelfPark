...
public class UsuarioController {
    ...
    public ResponseEntity<?> atualizarUsuario(
        ...
        try {
            ...
            Usuario usuarioExistente = usuarioServices.buscarPorEmail(email);

            // Verificar se o email está sendo alterado para um que já existe
            if (!email.equals(usuarioAtualizado.getEmail()) &&
                    usuarioServices.emailExiste(usuarioAtualizado.getEmail())) {
                return ResponseEntity.badRequest()
                        .body("O novo email já está em uso por outro usuário");
            }

            // Atualizar o usuário
            Usuario usuarioAtualizadoObj = usuarioServices.atualizarUsuario(email, usuarioAtualizado);
    ...
    // ADICIONAR: Atualizar parcial (PATCH) - para atualizações parciais
    @PatchMapping("/{email}")
    public ResponseEntity<?> atualizarParcialUsuario(
            @PathVariable String email,
            @RequestBody Usuario dadosParciais) {
        try {
            Usuario usuarioAtualizado = usuarioServices.atualizarParcialUsuario(email, dadosParciais);
